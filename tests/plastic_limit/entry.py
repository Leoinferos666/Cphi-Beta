...
from tests.plastic_limit.calculations import (
...
def render():
    ...
    if st.button("⬅ Back"):
        ...
        st.session_state.pop("active_pl_submission", None)

        st.rerun()

    st.title(
        f"Plastic Limit - {sample['sample_id']}"
    )
    ...
    submission = (
        ...
        .eq(
        "id",
        st.session_state["selected_pl_submission"]
        )
        .single()
        .execute()
    ).data
...
# # ==========================================
    ...
    if st.session_state.get("active_pl_submission") != current_submission:

        st.session_state["active_pl_submission"] = current_submission
        ...
        st.rerun()
    if "selected_pl_submission" not in st.session_state:
...
# ==========================================
    ...
    if saved_trials:
        ...
        st.session_state.pl_trial_count = max(
            DEFAULT_TRIALS,
            len(saved_trials)
        )
    ...
    if "pl_trial_count" not in st.session_state:

        st.session_state.pl_trial_count = DEFAULT_TRIALS

    trial_count = st.session_state.pl_trial_count
                ...
                if i >= DEFAULT_TRIALS:
                    ...
                    if st.button(
                       ...
                       key=f"delete_{i}",
                        ...
                        st.session_state.pl_trial_count -= 1
                        st.rerun()
    ...
    with c1:
        st.metric(
            "Plastic Limit",
            "--"
            if pl["plastic_limit"] is None
            else f"{pl['plastic_limit']:.2f} %"
        )
